...
resource "aws_launch_template" "executor" {
  ...
  image_id      = var.machine_image != "" ? var.machine_image : data.aws_region.current.id != "us-east-1" && data.aws_region.current.id != "us-west-1" && data.aws_region.current.id != "us-east-2" && data.aws_region.current.id != "us-west-1" && data.aws_region.current.id == "eu-west-2" ? aws_ami_copy.latest_ami.0.arn : data.aws_ami.latest_ami.0.image_id
